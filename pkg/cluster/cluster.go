...
import (
	...
	"github.com/nholuongut/footloose/pkg/config"
	"github.com/nholuongut/footloose/pkg/docker"
	"github.com/nholuongut/footloose/pkg/exec"
	"github.com/nholuongut/footloose/pkg/ignite"
)
...
func (c *Cluster) createMachineRunArgs(machine *Machine, name string, i int) []string {
	runArgs := []string{
		"-it",
		"--label", "works.nholuongut.owner=footloose",
		"--label", "works.nholuongut.cluster=" + c.spec.Cluster.Name,
		"--name", name,
